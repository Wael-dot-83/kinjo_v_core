...
def get_manager_kpi(
    ...
    from models import AttendanceLog, Child, EnrollmentApplication, EnrollmentStatus, Incident

    today = date.today()
    d_from = date.fromisoformat(from_date) if from_date else today - timedelta(days=29)
    ...
    child_ids = [e.child_id for e in active_enrollments]
    total_children = len(child_ids)

    # Supervisors
    supervisors = (
        db.query(User)
        .filter(User.role == UserRole.SUPERVISOR, User.kindergarten_id == kg_id, User.deleted_at.is_(None))
        .all()
    )
    ...
    supervisor_stats = []
    for sup in supervisors:
        from rbac import get_supervisor_child_ids
        sup_child_ids = list(get_supervisor_child_ids(sup.id, db))
        sup_reports = (
            db.query(DailyReport)
            .filter(DailyReport.child_id.in_(sup_child_ids), DailyReport.date >= d_from, DailyReport.date <= d_to)
            .count()
        ) if sup_child_ids else 0
        supervisor_stats.append({"id": sup.id, "name": sup.full_name or sup.username, "reports": sup_reports})

    # Attendance rate
    days = (d_to - d_from).days + 1
    expected_attendance = total_children * days
    actual_attendance = (
        db.query(AttendanceLog)
        .filter(AttendanceLog.child_id.in_(child_ids), AttendanceLog.date >= d_from, AttendanceLog.date <= d_to)
        .count()
    ) if child_ids else 0
    attendance_rate = round(actual_attendance / expected_attendance * 100, 1) if expected_attendance else 0

    # Report completion
    total_reports = (
        db.query(DailyReport)
        .filter(DailyReport.child_id.in_(child_ids), DailyReport.date >= d_from, DailyReport.date <= d_to)
        .count()
    ) if child_ids else 0
    report_rate = round(total_reports / expected_attendance * 100, 1) if expected_attendance else 0

    # Incidents
    incident_count = (
        db.query(Incident)
        .filter(Incident.child_id.in_(child_ids), Incident.occurred_at >= datetime(d_from.year, d_from.month, d_from.day))
        .count()
    ) if child_ids else 0
    ...
    return {
        ...
        "messaging": {
            "sent": messages_sent,
            "read": messages_read,
            "read_rate": round(messages_read / messages_sent * 100, 1) if messages_sent else 0,
        },
    }
