...
class TestAuthenticationIntegration:
    ...
    def test_full_registration_to_login_flow(self, client, test_db):
        ...
        login_response = client.post(
            "/token", data={"username": registration_data["email"], "password": registration_data["password"]}
        )
    ...
    def test_token_expiration(self, client, admin_user):
        ...
        with patch("auth.settings.ACCESS_TOKEN_EXPIRE_MINUTES", 0):
            response = client.post("/token", data={"username": "testadmin", "password": "Admin123!"})
    ...
    def test_role_based_endpoint_access(
        self, client, auth_headers_admin, auth_headers_manager, auth_headers_supervisor, auth_headers_parent
    ):
        ...
        response = client.get(
            ...
            params={"kindergarten_id": 1, "period_start": "2026-01-01", "period_end": "2026-01-31"},
        )
...
class TestEnrollmentWorkflowIntegration:
    ...
    def test_full_enrollment_workflow(
        ...
        response = client.post("/api/enrollment/apply", headers=auth_headers_parent, json=enrollment_data)
        ...
        response = client.post(f"/api/enrollment/{enrollment_id}/submit", headers=auth_headers_parent)
        assert response.status_code == 200
        ...
        response = client.post(
            f"/api/enrollment/{enrollment_id}/review", headers=auth_headers_manager, params={"decision": "accept"}
        )
        assert response.status_code == 200

    def test_enrollment_age_validation(self, client, auth_headers_parent, sample_kindergarten):
        ...
        response = client.post("/api/enrollment/apply", headers=auth_headers_parent, json=too_young_data)
        ...
        response = client.post("/api/enrollment/apply", headers=auth_headers_parent, json=too_old_data)
        assert response.status_code == 400

    def test_duplicate_enrollment_prevention(
        self, client, test_db, parent_user, auth_headers_parent, sample_kindergarten
    ):
...
class TestAttendanceIntegration:
    ...
    def test_complete_attendance_day(
        self, client, test_db, auth_headers_manager, sample_kindergarten, sample_child, sample_class, manager_user
    ):
        ...
        assignment = models.SupervisorAssignment(
            class_id=sample_class.id, supervisor_id=manager_user.id, is_primary=True, start_date=date.today()
        )
        ...
        checkin_response = client.post(
            ...
            params={"child_id": sample_child.id, "method": "pin", "dropped_by_name": "الأم - فاطمة"},
        )
        ...
        checkout_response = client.post(
            ...
            headers=auth_headers_manager,
            params={"child_id": sample_child.id, "picked_by_name": "الأب - أحمد"},
        )
    ...
    def test_checkout_without_checkin_fails(
        self, client, test_db, auth_headers_manager, sample_child, sample_kindergarten, sample_class, manager_user
    ):
        ...
        response = client.post(
            "/api/attendance/check-out", headers=auth_headers_manager, params={"child_id": sample_child.id}
        )
        assert response.status_code == 400
...
class TestDailyReportsIntegration:
    ...
    def test_complete_daily_report_workflow(
        ...
        assignment = models.SupervisorAssignment(
            class_id=sample_class.id, supervisor_id=supervisor_user.id, is_primary=True, start_date=date.today()
        )
        test_db.add(assignment)
        ...
        with patch("api.daily_reports_routes.datetime") as mock_dt, patch("validators.datetime") as validators_dt:
            mock_dt.now.return_value = morning_time
            mock_dt.side_effect = lambda *args, **kw: datetime(*args, **kw)
            validators_dt.now.return_value = morning_time
            validators_dt.side_effect = lambda *args, **kw: datetime(*args, **kw)

            create_response = client.post(
                "/api/daily-reports/create", headers=auth_headers_supervisor, json=report_data
            )
            ...
            # Submit report
            submit_response = client.post(f"/api/daily-reports/{report_id}/submit", headers=auth_headers_supervisor)
            assert submit_response.status_code == 200
            assert submit_response.json()["status"] == "submitted"

        # Approve report
        approve_response = client.post(f"/api/daily-reports/{report_id}/approve", headers=auth_headers_manager)
        assert approve_response.status_code == 200
        assert approve_response.json()["status"] == "approved"

    def test_parent_sees_only_approved_reports(self, client, test_db, parent_user, auth_headers_parent, sample_child):
...
class TestSafetyIncidentsIntegration:
    ...
    def test_incident_creation_with_followup(
        self, client, test_db, auth_headers_manager, sample_kindergarten, sample_class, sample_child
    ):
        ...
        response = client.post("/api/incidents", headers=auth_headers_manager, json=incident_data)
...
class TestKPIGovernanceIntegration:
    ...
    def test_attendance_rate_calculation(self, client, test_db, auth_headers_manager, sample_kindergarten):
    ...
    def test_governance_score_calculation(self, client, test_db, auth_headers_admin, sample_kindergarten):
...
class TestSupervisorOperationsIntegration:
    ...
    def test_supervisor_assignment_workflow(
        self, client, test_db, manager_user, supervisor_user, auth_headers_manager, sample_class
    ):
    ...
    def test_supervisor_assignment_uniqueness_enforced(
        self, client, test_db, manager_user, supervisor_user, auth_headers_manager, sample_class, sample_kindergarten
    ):
    ...
    def test_observation_recording(
        self, client, test_db, supervisor_user, auth_headers_supervisor, sample_class, sample_child, sample_kindergarten
    ):
    ...
    def test_supervisor_observation_access_blocked_for_unassigned_child(
        ...
        list_resp = client.get(f"/api/children/{other_child.id}/observations", headers=auth_headers_supervisor)
...
class TestMultiTenancyIsolation:
    ...
    def test_kindergarten_data_isolation(
        self, client, test_db, supervisor_user, auth_headers_supervisor, sample_kindergarten
    ):
        ...
        response = client.get("/api/supervisor/my-classes", headers=auth_headers_supervisor)
        assert response.status_code == 200
...
class TestConcurrentOperations:
    ...
    def test_concurrent_enrollment_handling(self, client, test_db, sample_kindergarten):
        """Verify concurrent enrollment attempts are handled correctly"""
        # This test documents expected behavior for production
        pass

    def test_concurrent_checkin_handling(self, client, test_db, sample_child):
...
class TestDataIntegrity:
    ...
    def test_audit_log_created_on_sensitive_operations(self, client, test_db, auth_headers_admin):
...
class TestAPIPerformance:
    ...
    def test_dashboard_response_time(self, client, test_db, supervisor_user, auth_headers_supervisor, sample_class):
        ...
        response = client.get("/api/supervisor/dashboard", headers=auth_headers_supervisor)
