...
class AgencyReportsService:
    ...
    def _logo_meta(self, code: str) -> dict[str, Any]:
        """Delegates to the centralized registry helper (single source)."""
        return agency_logo_meta(code)

    def catalog(self) -> dict[str, Any]:
        agencies = []
        # Official scope only, in a stable display order. Non-official agencies
        # (e.g. mopic) remain in AGENCY_REPORT_REGISTRY and reachable via direct
        # API/report routes but are intentionally excluded from the public
        # Official Agency Reports catalog/summary.
        for code in AGENCY_DISPLAY_ORDER:
            agency = AGENCY_REPORT_REGISTRY.get(code)
            if not agency or not agency.get("is_official", True):
                continue
            reports = []
            ...
            agencies.append({
                ...
                "description_en": agency.get("description_en"),
                "icon": agency.get("icon", "bi-bank"),
                "is_official": True,
                "last_updated": _now_iso(),
                "logo": self._logo_meta(code),
            ...
            "agencies": [
                {
                    ...
                    "name_en": a["name_en"],
                    "icon": a["icon"],
                    "is_official": a.get("is_official", True),
                    "logo": a["logo"],
    ...
    def reports_for_agency(self, agency_code: str) -> dict[str, Any]:
        ...
        catalog_agencies = self.catalog()["agencies"]
        agency_payload = next((a for a in catalog_agencies if a["code"] == agency_code), None)
        reports = agency_payload["reports"] if agency_payload else []
        return {
            ...
            "description_ar": agency.get("description_ar"),
            "description_en": agency.get("description_en"),
            "logo": self._logo_meta(agency_code),
            "reports": reports,
        }
    ...
    def _clean_custom_geo(self, scope: dict[str, Any]) -> dict[str, Any]:
        ...
        if gov not in (None, "", "null"):
            try:
                from services.jordan_locations import normalize_governorate, normalize_area, get_governorate_by_name, GOVERNORATES
                gov_obj = get_governorate_by_name(str(gov).strip())
                if gov_obj:
                    out["governorate"] = gov_obj["name_ar"]
                else:
                    out["governorate"] = normalize_governorate(str(gov).strip()) or str(gov)
            except Exception:
                out["governorate"] = str(gov)
        if city not in (None, "", "null"):
            try:
                from services.jordan_locations import normalize_area
                gov_key = None
                if out.get("governorate"):
                    for g in GOVERNORATES:
                        if g["name_ar"] == out["governorate"]:
                            gov_key = g["key"]
                            break
                if gov_key:
                    normalized_city = normalize_area(gov_key, str(city).strip())
                    out["city"] = normalized_city or str(city)
                else:
                    out["city"] = str(city)
            except Exception:
                out["city"] = str(city)
