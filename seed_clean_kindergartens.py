...
SYNTHETIC_KGS = [
    ("روضة المفرق الحديثة",  "Mafraq Modern KG",       "المفرق",  "المفرق",  "المركز",  "شارع الأردن"),
    ("حضانة الأمل - المفرق", "Al Amal Nursery Mafraq", "المفرق",  "المفرق",  "الغربي",  "شارع الوحدة"),
    ("روضة الريحان",          "Al Raihan KG",           "المفرق",  "رحاب",    "الشمالي", "شارع الملك"),
    ("حضانة النجمة - المفرق", "Al Najma Mafraq",        "المفرق",  "المفرق",  "المركز",  "شارع القدس"),
    ("روضة السنابل - المفرق", "Al Sanabel Mafraq",      "المفرق",  "المفرق",  "الجنوبي", "شارع الحرية"),
    ("روضة جرش الأهلية",     "Jerash National KG",      "جرش",     "جرش",     "المدينة", "شارع الملك حسين"),
    ("حضانة الروابي - جرش",  "Al Rawabi Jerash",        "جرش",     "جرش",     "الجنوبي", "شارع الاستقلال"),
    ("روضة الزيتون",          "Al Zaytoun KG",          "جرش",     "برما",    "الشرقي",  "شارع الوحدة"),
    ("حضانة الفرح - جرش",    "Al Farah Jerash",         "جرش",     "جرش",     "الشمالي", "شارع العلوم"),
    ("روضة العلياء - جرش",   "Al Ulia Jerash",          "جرش",     "جرش",     "المركز",  "شارع الأردن"),
    ("روضة الطفيلة الوطنية", "Tafileh National KG",     "الطفيلة", "الطفيلة", "المركز",  "شارع الملك حسين"),
    ("حضانة البتراء",         "Al Batra Nursery",       "الطفيلة", "الطفيلة", "الغربي",  "شارع القدس"),
    ("روضة الحوراء",          "Al Hawraa KG",           "الطفيلة", "بصيرا",   "المدينة", "شارع الوحدة"),
    ("حضانة الأجيال - طفيلة","Al Ajyal Tafileh",        "الطفيلة", "الطفيلة", "الشمالي", "شارع الأردن"),
    ("روضة معان الحديثة",    "Maan Modern KG",          "معان",    "معان",    "المركز",  "شارع الملك عبدالله"),
    ("حضانة الواحة - معان",  "Al Waha Maan",            "معان",    "معان",    "الغربي",  "شارع الاستقلال"),
    ("روضة القوافل",          "Al Qawafil KG",          "معان",    "الشوبك",  "المدينة", "شارع الحسين"),
...
def seed_kindergartens(db: Session) -> List[Kindergarten]:
    ...
    for row in rows[1:]:
        name_ar = (str(row[col["اسم الروضة (عربي)"]]).strip() if row[col["اسم الروضة (عربي)"]] else "")
        ...
        if (name_ar, gov) in existing:
            continue
        name_en = (str(row[col["اسم الروضة (إنجليزي)"]]).strip() if row[col["اسم الروضة (إنجليزي)"]] else name_ar)
