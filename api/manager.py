...
def get_manager_dashboard(current_user: models.User = Depends(require_manager), db: Session = Depends(get_db)):
    ...
    if kindergarten_id is None:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="No kindergarten is associated with this account"
        )
    ...
    if kindergarten is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Associated kindergarten not found")
    ...
    attendance_today = (
        db.query(func.count(func.distinct(models.AttendanceLog.id)))
        .join(models.Child)
        .join(models.EnrollmentApplication)
        .filter(
            models.EnrollmentApplication.kindergarten_id == kindergarten_id,
    ...
    recent_incidents = (
        ...
        .filter(models.Incident.kindergarten_id == kindergarten_id, func.date(models.Incident.occurred_at) >= week_ago)
        .scalar()
        or 0
    )
    ...
    classes_without_supervisor = [
        {"id": c.id, "name_ar": c.name_ar, "name_en": c.name_en} for c in classes if c.id not in covered_class_ids
    ]
    ...
    pending_absence_requests = (
        db.query(func.count(models.AbsenceRequest.id))
        .filter(
    ...
    dashboard = {
        ...
        "charts": {"attendance": attendance_trend, "enrollment": enrollment_breakdown},
