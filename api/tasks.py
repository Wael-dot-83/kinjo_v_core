...
def create_task(
    task_data: TaskCreate, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Create a new task"""
    ...
    if current_user.role == models.UserRole.ADMIN:
        if task_data.kindergarten_id:
            kindergarten = (
                db.query(models.Kindergarten).filter(models.Kindergarten.id == task_data.kindergarten_id).first()
            )
            if not kindergarten:
            ...
            if not kindergarten:
                raise HTTPException(
                    status_code=400, detail="No active kindergarten found; please provide kindergarten_id"
                )
            kindergarten_id = kindergarten.id
    else:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid priority: {task_data.priority}. Valid values: LOW, MEDIUM, HIGH, URGENT"
        )
    ...
    if task_data.status is not None:
        try:
            ...
            if new_status == models.TaskStatus.COMPLETED:
                task.completed_at = datetime.now(_JORDAN_TZ)
        except ValueError:
