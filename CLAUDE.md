...
- Do not work directly on `main`, and do not edit the primary checkout (`d:\Final Version`) — it is shared with other agents. Use your own git worktree and feature branch, keep the human git author identity but add `Agent:` / `Agent-Run-ID:` commit trailers, verify **your own** worktree is clean before rebasing/merging/switching, and remove the worktree when the branch **lands or is abandoned** (`git worktree remove` + `git worktree prune`). Never leave a worktree holding uncommitted work — and note `git diff HEAD > rescue.patch` is NOT a rescue: it silently drops untracked files and conflict stages (it does capture staged content, just not which files were staged). See AGENTS.md "Agent isolation" for the full rescue package and the 72-hour staleness rule — this replaces the old "no worktrees, work in the root" rule, which caused concurrent agents to collide in one tree.
- Do not commit generated files or diagnostics (route dumps, audit snapshots) unless explicitly required; historical reports go in `docs/reports/`, manual scripts in `scripts/manual-diagnostics/`
